...
void bottom_view_helper(Node *root, int &_min, int &_max, unordered_map<int, vi> &vertical_order){
    if(root == NULL)
        return;
    ...
    while(!q.empty()){
        ...
        vertical_order[front.second].__emplace_back(front.first-> data);
...
void bottom_view(Node *root){
    ...
    unordered_map<int, vi> vertical_order;
    bottom_view_helper(root, _min, _max, vertical_order);
    for(int i = _min; i <= _max; i++)
        cout << vertical_order[i][vertical_order[i].size() - 1] << " ";
    cout << '\n';
}
