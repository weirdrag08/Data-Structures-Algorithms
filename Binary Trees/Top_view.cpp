...
void top_view_helper(Node *root, int &_min, int &_max, unordered_map<int, vi> &vertical_order){
    if(root == NULL)
        return;
...
void top_view(Node *root){
    ...
    top_view_helper(root, _min, _max, vertical_order);
    for(int i = _min; i <= _max; i++){
       cout << vertical_order[i][0] << " ";
    }
    cout << '\n';
}
