...
Node* construct_BST(int s, int e, vi &inorder){
    if(s > e)
        return NULL;
    int mid = s + (e - s)/2; 
    Node *left_tree = NULL, *right_tree = NULL;
    Node *root = new Node(inorder[mid]);
    left_tree = construct_BST(s, mid - 1, inorder);
    right_tree = construct_BST(mid + 1, e, inorder);
    root-> left = left_tree;
...
int main(){
    ...
    for(int i = 0; i < n; i++)
        cin >> inorder[i];
    Node *root = construct_BST(0, n - 1, inorder);
