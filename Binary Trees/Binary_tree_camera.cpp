...
int apply_cameras(Node *root, int &cameras){
    if(root == NULL)
        return 0;
    int leftside = 0, rightside = 0;
    if(root-> left != NULL)
        leftside = apply_cameras(root-> left, cameras);
    if(root-> right != NULL)
        rightside = apply_cameras(root-> right, cameras);
...
int main(){
    ...
    Node *root = input();
    int cameras = 0;
    int val = apply_cameras(root, cameras);
    val == -1 ? cameras++ : cameras = cameras;
    cout << cameras << '\n';
}